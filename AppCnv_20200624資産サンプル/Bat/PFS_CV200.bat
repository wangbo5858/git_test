...
rem ----------------------------------------
set ARG_DATE=%1
if "%ARG_DATE%" == "" (set ARG_DATE="*")
echo %DATE% %TIME% "指定日：%ARG_DATE%" >> "%LOG_PATH%\%PKEY%.log"
...
sqlplus %CONNECT_MAIN% @%PKEY% %SCHEMA_PPS% %SCHEMA_PSI% %COMCD% %DIVCD% %PLTCD% %ARG_DATE% >> "%LOG_PATH%\%PKEY%.log"
